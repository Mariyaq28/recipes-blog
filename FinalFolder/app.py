...
def init_db_and_categories():
    ...
    if not _db_initialized:
        # Avoid running db.create_all() if database is default SQLite on Vercel (read-only filesystem)
        if not (os.environ.get('VERCEL') and app.config['SQLALCHEMY_DATABASE_URI'].startswith('sqlite:///recipes.db')):
            try:
                db.create_all()
    ...
    try:
        ...
        app.logger.error(f"Categories initialization failed: {e}")
